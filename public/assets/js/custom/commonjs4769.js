...
function onDocumentReady(isDataTable, isShortText) {
    ...
    if (strGlobalSlotEpoch == null || strGlobalSlotEpoch == "null" || strGlobalSlotEpoch == "Slot") {
        $(".switch-slot-epoch").text("Slot");
        $(".switch-slot-epoch").attr("title", "Click to show Epoch");
        $(".switch-slot-epoch").attr("data-bs-title", "Click to show Epoch");
        $(".showSlotNumber").show();
        $(".showEpochNumber").hide();
    } else if (strGlobalSlotEpoch == "Epoch") {
        $(".switch-slot-epoch").text("Epoch");
        $(".switch-slot-epoch").attr("title", "Click to show Slot");
        $(".switch-slot-epoch").attr("data-bs-title", "Click to show Slot");
        $(".showSlotNumber").hide();
        $(".showEpochNumber").show();
    }

    if (!isDataTable) {
        renderFnTooltip($("#lnkAgeDateTime"), 0)
        renderFnTooltip($(".switch-txn-fee-gas-price"), 0)
        renderFnTooltip($(".switch-slot-epoch"), 0)
    }
...
function onAddressDocReady() {
    if (strGlobal == "" || strGlobal == strAge) {
        ...
        $("#lnkDeposithBlkAgeDateTime").attr("title", strAgeTitle);
...
function clearGlobalValue() {
    strGlobal = "";
    document.cookie = "etherscan_switch_age_datetime=" + strGlobal;
}
...
function switchSlotAndEpoch(self) {
    disposeFnTooltip(self ? $(self) : $(".switch-slot-epoch"))

    var strVal = self ? $(self).text() : $(".switch-slot-epoch").text();
    if (strVal == "Slot") {
        $(".switch-slot-epoch").text("Epoch");
        $(".switch-slot-epoch").attr("title", "Click to show Slot");
        $(".switch-slot-epoch").attr("data-bs-title", "Click to show Slot");
        $(".showEpochNumber").show();
        $(".showSlotNumber").hide();
        sessionStorage.setItem("ShowSlotEpoch", "Epoch");
    }
    else if (strVal == "Epoch") {
        $(".switch-slot-epoch").text("Slot");
        $(".switch-slot-epoch").attr("title", "Click to show Epoch");
        $(".switch-slot-epoch").attr("data-bs-title", "Click to show Epoch");
        $(".showEpochNumber").hide();
        $(".showSlotNumber").show();
        sessionStorage.setItem("ShowSlotEpoch", "Slot");
    }

    if (self) {
        renderFnTooltip($(self), 1)
    } else {
        renderFnTooltip($(".switch-slot-epoch"), 1)
    }
}
...
function initDateTimeTooltip(useShortHeader = false) {
    ...
    let headerText = "";
    let tooltipTitle = "";
    const datetimeTextCookie = getCookie("etherscan_switch_age_datetime");
...
function convertUtcTimestampToLocalTime() {
    ...
    if (strDateTimeFormat === "LOCAL") {
        ...
        if ($(localDateTimeClass).length > 0) {
            ...
            $localDatetimeNFTColumn.each(function (i, v) {
                ...
                var html = v.innerHTML;
                var content = html.replace(dateText, localDateTimeNFT);
                localDateTimeArray.push(content);
...
function switchAmountToValue(el, text1, text2, isStart) {
    if (el) {
        var cAmountPrice = getCookie("etherscan_switch_token_amount_value");
        ...
        if ((isStart == false && cAmountPrice == "value") || (isStart == true && cAmountPrice == "amount")) {
            ...
            $(".td_showValue").show()
            $(".td_showAmount").hide()
            writeCookie("etherscan_switch_token_amount_value", "amount")
        } else {
            $(el).html(text2)
            $("#tooltipAmountValue").hide()
            $(".td_showValue").hide()
            $(".td_showAmount").show()
            writeCookie("etherscan_switch_token_amount_value", "value")
        }
    }
}
...
function getTruncatedAddress(strAddress) {
    var strVal = "";
    if (strAddress !== "") {
        var strTruncateType = getCookie('etherscan_address_format');
        if (strTruncateType === "") {
            strTruncateType = "0";
        }
